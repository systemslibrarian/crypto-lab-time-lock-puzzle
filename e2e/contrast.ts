 ...
 *    `color-mix()`. Every action button on this page is filled with
 *    `color-mix(in srgb, var(--accent) 22%, transparent)` (34% on hover, and
 *    the danger and trapdoor buttons mix their own hues), the hero's "why it
 *    matters" aside is a `color-mix(in oklab, …)`, and the shared header's ink
 *    is another. axe files the text on all of them under `incomplete` rather
 *    than measuring it, which is most of the page's controls.
 ...
 *    `opacity:.85`. Disabled action buttons drop to `.45`, but WCAG 1.4.3
 *    exempts inactive components and this helper honours that (see `inactive`),
 *    so they are skipped rather than reported.
 *    Note this lab's reduced-motion block sets `animation: none` on `.panel`
 *    and `transition: none` on `.progress-fill`. Neither cancels an end state:
 *    the `fade` keyframe runs `opacity 0 → 1` with the default `fill-mode:
 *    none`, so a panel that never animates simply renders at its own
 *    `opacity: 1`.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. Every `.mono-box`
 *    here is `overflow-x: auto; white-space: nowrap` — that is nine of them
 *    once both parameter tables have rendered — and `.progress-track` is
 ...
 *    has no SVG text at all — its only SVG is the shared header's hamburger and
 *    GitHub marks, both `aria-hidden` and glyph-free — so the sibling walk finds
 *    nothing to do here. It is kept because the alternative is a helper that
 ...
 *
 * ONE KNOWN BOUNDARY, stated rather than papered over. `sampleLayer` evaluates
 * a `background-image` layer across the whole border box; it does not honour
 * `background-position`, `background-size` or `background-repeat: no-repeat`.
 * The only positioned background layers on this page are the two 0.36rem
 * `linear-gradient` triangles that draw the custom `<select>` chevron, and no
 * `<select>` is ever measured here — a `<select>`'s child nodes are `<option>`
 * elements, so it owns no text node of its own, and a closed select's options
 * have a zero-sized box. Its own label text is a plain opaque pair anyway:
 * `--text` on `--code-bg` is #e6eaf4 on #0b0e15 in dark and #16203a on #f3f6fd
 * in light, both far past AA. If a positioned background layer is ever added
 * behind real text, this helper will need position/size handling first.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: every action button's
     * fill is authored with `color-mix()`, and Chromium reports that to
     ...
     * `.progress-fill`'s `linear-gradient(90deg, color-mix(--accent 60%,
     * transparent), var(--accent))`: at its worst stop it reports the track
     * behind it for a bar that is fully saturated at its right-hand end. So each
     ...
     *
     * A driven scan here walks the six tab panels' prose, both parameter tables,
     * the four solve stats, the status banner and the reveal block, and every
     * one of those nodes re-walks the same handful of ancestors up to `<body>`.
     * Without the caches the pass re-reads the same computed styles and rects
     * thousands of times, and the gate scans a couple of dozen states per
     ...
     * that is every `.mono-box` and `.progress-track`.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has no <details>; what it does have is five
      // inactive `.panel`s at `display: none` (only `[data-active='true']` is
      // shown) plus `#createOut` and `#reveal`, each toggled through inline
      // `style.display`. `checkVisibility()` catches every one of those; the
      // gate clicks each tab and drives each block into its revealed state and
      // scans it there, rather than forcing them all visible at once — a
      // document with all six panels open is one no visitor can load.
     ...
     *
     * This lab's remaining `aria-hidden` marks are the status-banner emoji
     * (each paired with a real sentence), the `.progress-track` (whose value is
     * printed as text in `#statPct`), and the shared header's two icons. Note
     * that `#chain` used to be in that list and is NOT any more: it prints the
     * live chain value `xₙ = …`, which is real output no other element shows,
     * and hiding it meant neither this oracle nor axe ever measured its
     * `--accent` ink — which was 2.77:1 in light theme. Skipping `aria-hidden`
     * text is only safe while `aria-hidden` is used for genuinely decorative
     * marks.
     */
     ...
     *
     * This lab has no SVG figure at all today — only the shared header's two
     * icon marks. The guard stays because the mistake is cheap to make and
     * silent: interpolating a string ARRAY into a template literal
     ...
     * measures them reports a 1:1 failure against the card — a ratio describing
